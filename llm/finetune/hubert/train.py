...
from mindnlp.transformers import HubertForCTC, Wav2Vec2Processor, Wav2Vec2CTCTokenizer
...
def show_param_cnt(model:HubertForCTC):
...
def run_debug(args):
    ...
    def test_infer():
        ...
        input_values, labels = next(iter(dataset))
        inputs = {
            'input_values': input_values,
            'labels': labels,
        }
        print('truth_ids:', labels)
        for it in labels:
...
def get_args():
    ...
    parser.add_argument('-M', '--model', default='facebook/hubert-large-ll60k', help='model hub from hugging face')
    parser.add_argument('-P', '--processor', default='facebook/hubert-large-ls960-ft', help='processor hub from hugging face')
